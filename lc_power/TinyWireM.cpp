...
uint8_t USI_TWI::endTransmission(){ // actually sends the buffer
  bool xferOK = false;
  uint8_t errorCode = 0;
  xferOK = usiTwiStartTransceiverWithData(USI_Buf,USI_BufIdx+1); // core func that does the work
  USI_BufIdx = 0;
  if (xferOK) return 0;
  else {                                  // there was an error
    errorCode = usiTwiGetStateInfo(); // this function returns the error number
    return errorCode;
  }
}
...
uint8_t USI_TWI::requestFrom(uint8_t slaveAddr, uint8_t numBytes){ // setup for receiving from slave
  ...
  xferOK = usiTwiStartTransceiverWithData(USI_Buf,numBytes); // core func that does the work
  // USI_Buf now holds the data read
  if (xferOK) {
    /*
    errorCode = usiTwiMasterStop();
    if (errorCode == 0) {
      errorCode = usiTwiGetStateInfo();
      return errorCode;
    }
    */
    return 0;
